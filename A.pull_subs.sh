...
elsfp=/Volumes/group/active/ELS/ELS-T3
elsfp2=/Volumes/group/active/ELS/ELS-T4 #/Volumes/iang/ELS_data/ELS-T3
...
elst1fp2=/Volumes/iang/ELS_data/ELS_DTI_Analysis/ELS_T4/matproc
...
for sub in 164-T3 176-T3 188-T3 307-T3  ; do
  if [ -e $sub/qt1_pe0.nii.gz ] ; then
    if [ -e $sub/qt1_pe1.nii.gz ] ; then
      if [ -e $destfp/${sub}/raw/qt1_pe1.nii.gz ] ; then
        echo '--- qT1 already copied for' $sub '---'
...
for sub in 112-T4 072-T4 ; do
  if [ -e $sub/qt1_pe0.nii.gz ] ; then
    if [ -e $sub/qt1_pe1.nii.gz ] ; then
      if [ -e $destfp/${sub}/raw/qt1_pe1.nii.gz ] ; then
        ...
        mkdir $destfp/${sub}
        mkdir $destfp/${sub}/raw
        cp $elsfp2/${sub}/qt1_pe1.nii.gz $destfp/${sub}/raw/.
        cp $elsfp2/${sub}/qt1_pe0.nii.gz $destfp/${sub}/raw/.
        echo '<><><> qT1 copied over to proc/qT1/subjDir for' $sub '<><><>'
      fi
    else
      echo '*** qT1 pe1 not found for' $sub '***'
  ...
  if [ -e $elst1fp2/els${sub}/*_t1_acpc.nii.gz ] ; then
    if [ -d $destfp/${sub} ] ; then
      if [ -e $destfp/${sub}/raw/T1_acpc.nii.gz ] ; then
        echo '--- ACPC aligned T1 already copied for' $sub '---'
      else
        cp $elst1fp2/els${sub}/*_t1_acpc.nii.gz $destfp/${sub}/raw/T1_acpc.nii.gz
        echo '<><><> Copied T1w ACPC aligned for' $sub '<><><>'
      fi
    else
      echo 'Analysis folder does not exist for '$sub
